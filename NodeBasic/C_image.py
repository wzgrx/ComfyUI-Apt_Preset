...
class Image_transform_sum:
    ...
    def process_mask(self, mask, feather_amount, mask_blur, mask_expansion):
        if mask_expansion != 0:
    ...
    def apply_color_adjust_HSLments(self, image, brightness, contrast, saturation, hue, sharpness):
        if brightness != 1.0:
    ...
    def apply_toon_filter(self, image, strength):
        ...
        gray = cv2.cvtColor(img_np, cv2.COLOR_GRAY2GRAY)
    ...
    def process_image(self, 
                    bg_mode, bg_color, blending_mode,
                    blend_strength, front_img_sacle, x_position, y_position,
                    rotation, opacity, edge_detection,
                    edge_thickness, edge_color, 
                    mask_blur, mask_expansion,
                    front_img=None, bg_img=None, front_mask=None, output_format="RGBA", only_mask=False, custom_model_path=""):
        ...
        def process_single_image(fg_image, bg_image=None, input_mask=None, frame_number=0):
            ...
            final_mask = self.process_mask(final_mask, feather_amount, mask_blur, mask_expansion)
            ...
            elif bg_mode == "image" and bg_pil is not None:
                result = bg_pil.convert("RGBA").resize((new_width, new_height), Image.LANCZOS)
            else:
                result = Image.new("RGBA", (new_width, new_height), (0, 0, 0, 0))

            fg_width = int(new_width * front_img_sacle)
            fg_height = int(new_height * front_img_sacle)
            ...
            # Apply the same transformations to the mask as the image
            fg_mask_resized = fg_mask.resize((fg_width, fg_height), Image.LANCZOS)
            fg_mask_rotated = fg_mask_resized.rotate(rotation, resample=Image.BICUBIC, expand=True)

            # Create a blank image for mask placement
            mask_result = Image.new("L", result.size, 0)  # L: grayscale mode
            mask_result.paste(fg_mask_rotated, (paste_x, paste_y), fg_mask_rotated)
            invert_mask = ImageOps.invert(mask_result)

            return pil2tensor(result), pil2tensor(mask_result), pil2tensor(invert_mask)

        try:
            fg_batch_size = front_img.shape[0]
            bg_batch_size = bg_img.shape[0] if bg_img is not None else 0
            ...
            for frame_number in tqdm(range(animation_frames), desc="Processing frames"):
                ...
                single_fg = front_img[fg_index:fg_index+1]
                single_bg = bg_img[bg_index:bg_index+1] if bg_index is not None else None
                single_input_mask = front_mask[fg_index:fg_index+1] if front_mask is not None else None
                
                processed_image, processed_mask, invert_mask = process_single_image(single_fg, single_bg, single_input_mask, frame_number)
            ...
            if stacked_masks.max() > 1:
                stacked_masks = stacked_masks / 255.0
            invert_mask = torch.clamp(1.0 - stacked_masks, min=0.0, max=1.0)

            return (stacked_images, stacked_masks, invert_mask)
