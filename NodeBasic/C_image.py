...
class Image_Pair_Merge:
    ...
    def composite2(self, layout_mode, bg_mode, size_mode, target_size, divider_thickness, 
        ...
        if mask1_stack and mask1 is not None:
            ...
            separated_result = Mask_transform_sum().separate(bg_mode=bg_mode_stack, mask_mode=mask_mode, output_mask=output_mask, ignore_threshold=ignore_threshold, opacity=opacity, outline_thickness=outline_thickness, smoothness=smoothness, expand_width=expand_width, expand_height=expand_height, tapered_corners=tapered_corners, sum_mask_min=sum_mask_min, sum_mask_max=sum_mask_max, base_image=image1, mask=mask1_tensor)
        ...
        if mask2_stack and mask2 is not None: 
            ...
            separated_result = Mask_transform_sum().separate(bg_mode=bg_mode_stack, mask_mode=mask_mode, output_mask=output_mask, ignore_threshold=ignore_threshold, opacity=opacity, outline_thickness=outline_thickness, smoothness=smoothness, expand_width=expand_width, expand_height=expand_height, tapered_corners=tapered_corners, sum_mask_min=sum_mask_min, sum_mask_max=sum_mask_max, base_image=image2, mask=mask2_tensor)
        ...
        if mask1 is None:
            mask1 = torch.ones((image1.shape[0], image1.shape[1], image1.shape[2]), dtype=torch.float32) if len(image1.shape) == 3 else torch.ones((1, image1.shape[0], image1.shape[1]), dtype=torch.float32)
            
        if mask2 is None:
            mask2 = torch.ones((image2.shape[0], image2.shape[1], image2.shape[2]), dtype=torch.float32) if len(image2.shape) == 3 else torch.ones((1, image2.shape[0], image2.shape[1]), dtype=torch.float32)

        final_img_tensor, final_mask_tensor, box2, image2, mask2 = Pair_Merge().composite(layout_mode=layout_mode, bg_mode=bg_mode, size_mode=size_mode, target_size=target_size, divider_thickness=divider_thickness, image1=image1, image2=image2, mask1=mask1, mask2=mask2)
