...
class stack_Mask2color:
    ...
    def pack_mask_info(self, 
                      mask1_color, fill1, outline_thickness1, extrant_to_block1,
                      mask2_color, fill2, outline_thickness2, extrant_to_block2,
                      mask3_color, fill3, outline_thickness3, extrant_to_block3):
        mask_info = {
            "mask1": {
                "color_name": mask1_color.lower(),
                "rgb": self.colors.get(mask1_color.lower(), (255, 255, 255)),
                "fill": fill1,
                "outline_thickness": outline_thickness1,
                "extrant_to_block": extrant_to_block1
            },
            "mask2": {
                "color_name": mask2_color.lower(),
                "rgb": self.colors.get(mask2_color.lower(), (255, 255, 255)),
                "fill": fill2,
                "outline_thickness": outline_thickness2,
                "extrant_to_block": extrant_to_block2
            },
            "mask3": {
                "color_name": mask3_color.lower(),
                "rgb": self.colors.get(mask3_color.lower(), (255, 255, 255)),
                "fill": fill3,
                "outline_thickness": outline_thickness3,
                "extrant_to_block": extrant_to_block3
            }
        }
...
class create_Mask_visual_tag:
    ...
    def separate(self, mask, base_image, ignore_threshold=100, opacity=0.8, 
                outline_thickness=1, extrant_to_block=False, smoothness=1, 
                image_output=None, out_color="colorful", mask_info=None, 
                fill=True, prompt=None, extra_pnginfo=None):
        ...
        for i, contour in enumerate(sorted_contours):
            ...
            # 修改颜色处理逻辑
            # 当out_color为特定颜色时，使用统一颜色，忽略mask_info中的颜色设置
            if out_color in ["white", "black", "red", "green", "blue"]:
                ...
                extrant_to_block_current = extrant_to_block
            elif mask_info and f"mask{i+1}" in mask_info:
                # colorful模式下且有mask_info时，使用mask_info中的配置
                ...
                extrant_to_block_current = config.get("extrant_to_block", extrant_to_block)
            else:
                # colorful模式下且无mask_info时，使用彩色模式
...
def process_foreground_fit_background(foreground_img, background_img, scale_mode='fit_height', 
                                    user_scale=1.0, x_offset=0, y_offset=0, rotation=0):
    """
    将前景图像按指定模式缩放到背景图像上，并返回处理后的图像和边界框信息
    
    参数:
        foreground_img: PIL.Image - 前景图像
        background_img: PIL.Image - 背景图像
        scale_mode: str - 缩放模式 ('fit_height', 'fit_width', 'fit_min')
        user_scale: float - 用户缩放因子
        x_offset, y_offset: int - 前景在背景中的偏移量（相对于画布中心）
        rotation: int - 前景旋转角度（度）
    
    返回:
        composed_img: PIL.Image - 合成后的图像
        relative_box: tuple - 遮罩边界框 (center_x, center_y, width, height)
                        center_x, center_y: 遮罩中心相对于画布中心的坐标
                        width, height: 遮罩的宽和高
        foreground_mask: PIL.Image - 前景遮罩（与背景尺寸相同）
    """
    # 获取图像尺寸
    ...
    def calculate_scaled_size():
        if scale_mode == 'fit_height':
            # 按高度缩放
            target_height = bg_height
            target_width = int(target_height * (fg_width / fg_height))
        elif scale_mode == 'fit_width':
            # 按宽度缩放
            target_width = bg_width
            target_height = int(target_width * (fg_height / fg_width))
        else:  # 'fit_min'
            # 按最小边缩放，确保前景完全包含在背景内
            width_scale = bg_width / fg_width
            height_scale = bg_height / fg_height
            safe_scale = min(width_scale, height_scale)
            
            target_width = int(fg_width * safe_scale * user_scale)
            target_height = int(fg_height * safe_scale * user_scale)
    ...
    # 缩放前景图像
    scaled_fg = foreground_img.resize((target_width, target_height), resample=Image.Resampling.BICUBIC)
    
    # 旋转前景图像（如果需要）
    if rotation != 0:
        scaled_fg = scaled_fg.rotate(rotation, expand=True, resample=Image.Resampling.BICUBIC)
    
    # 计算前景在背景中的位置（相对于画布中心）
    bg_center_x, bg_center_y = bg_width // 2, bg_height // 2
    paste_x = bg_center_x - scaled_fg.size[0] // 2 + x_offset
    paste_y = bg_center_y - scaled_fg.size[1] // 2 + y_offset
    
    # 创建与背景相同尺寸的透明画布
    fg_with_alpha = Image.new('RGBA', (bg_width, bg_height), (0, 0, 0, 0))
    fg_with_alpha.paste(scaled_fg, (paste_x, paste_y))
    
    # 合成图像
    background_rgba = background_img.convert('RGBA')
    composed_img = Image.alpha_composite(background_rgba, fg_with_alpha).convert('RGB')
    
    # 创建前景遮罩
    foreground_mask = Image.new('L', (bg_width, bg_height), 0)
    mask_data = [(255 if p[3] > 0 else 0) for p in fg_with_alpha.getdata()]
    foreground_mask.putdata(mask_data)
    
    # 计算边界框（相对于画布中心）
    mask_center_x = paste_x + scaled_fg.size[0] // 2 - bg_center_x
    mask_center_y = paste_y + scaled_fg.size[1] // 2 - bg_center_y
    mask_width, mask_height = scaled_fg.size
    
    return composed_img, (mask_center_x, mask_center_y, mask_width, mask_height), foreground_mask



class XXXX无效create_Mask_match_shape: 
    def __init__(self):
        self.bg_colors = {
            "black": (0, 0, 0),
            "white": (255, 255, 255),
    ...
    def INPUT_TYPES(cls):
        return {
            "required": {
                ...
                "scale_mode": (["fit_height", "fit_width", "fit_min", "original"], {"default": "fit_min"}),
                "auto_scale_mode": (["原始", "按照宽", "按照高"], {"default": "原始"}),
            },
            "optional": {
                "fimg": ("IMAGE",),
                "fmask": ("MASK",),
    ...
    def compose(self, bimg, bmask, b_color, b_extrant_to_block, f_extrant_to_block, 
        ...
        mask2_np = np.zeros_like(bg_color_mask)  # 初始化mask2
        ...
        if has_foreground:
            ...
            if fmask is None:
                # 创建与前景同尺寸的全白遮罩
                fmask_pil = Image.new('L', fimg_pil.size, 255)
            else:
                # 转换输入的mask张量为PIL图像
            ...
            # 前景与遮罩的初始缩放（基于auto_scale_mode）
            f_original_width, f_original_height = fimg_pil.size
            
            # 基于auto_scale_mode的初始缩放
            if auto_scale_mode == "按照宽":
                target_width = bmask_width
                target_height = int(target_width * (f_original_height / f_original_width))
            elif auto_scale_mode == "按照高":
                target_height = bmask_height
                target_width = int(target_height * (f_original_width / f_original_height))
            else:  # 原始
                target_width, target_height = f_original_width, f_original_height
            
            # 应用初始缩放
            scale_ratio = target_width / f_original_width
            fimg_scaled = fimg_pil.resize(
                (int(f_original_width * scale_ratio), int(f_original_height * scale_ratio)),
                resample=resample_method
            )
            fmask_scaled = fmask_pil.resize(
                (int(f_original_width * scale_ratio), int(f_original_height * scale_ratio)),
                resample=resample_method
            )

            # 合并前景与遮罩
            fg_rgba = fimg_scaled.convert('RGBA')
            fg_rgba.putalpha(fmask_scaled)

            # 调用外部process_foreground_fit_background函数处理前景与背景的匹配
            composed_temp, _, fg_mask_pil = process_foreground_fit_background(
                foreground_img=fg_rgba,
                background_img=bimg_processed,
          ...
          # 平滑处理
            if f_smoothness > 0:
                fg_mask_np = np.array(fg_mask_pil)
                blurred_mask = cv2.GaussianBlur(fg_mask_np, (0, 0), sigmaX=f_smoothness)
                fg_mask_pil = Image.fromarray(blurred_mask)
                
                # 应用平滑后的遮罩到前景
                composed_temp_rgba = composed_temp.convert('RGBA')
                alpha = composed_temp_rgba.split()[-1]
                alpha = Image.fromarray(blurred_mask)
                composed_temp_rgba.putalpha(alpha)
                composed_temp = composed_temp_rgba.convert('RGB')

            # 透明度调整
            if f_opacity != 1.0:
                composed_temp_rgba = composed_temp.convert('RGBA')
                alpha = composed_temp_rgba.split()[-1]
                alpha = Image.eval(alpha, lambda x: int(x * f_opacity))
                composed_temp_rgba.putalpha(alpha)
                composed_temp = composed_temp_rgba.convert('RGB')

            # 边缘检测
            if edge_detection:
                edge_img = Image.new('RGBA', composed_temp.size, (0, 0, 0, 0))
                draw = ImageDraw.Draw(edge_img)
                edge_color_rgb = self.bg_colors.get(edge_color, (255, 255, 255))
                
                # 从前景遮罩提取边缘
                fg_mask_np = np.array(fg_mask_pil)
                _, binary = cv2.threshold(fg_mask_np, 1, 255, cv2.THRESH_BINARY)
                ...
                # 合并边缘图像
                composed_temp_rgba = composed_temp.convert('RGBA')
                composed_temp = Image.alpha_composite(composed_temp_rgba, edge_img).convert('RGB')

            # 更新最终图像和遮罩
            composed_pil = composed_temp
            final_mask_np = np.maximum(bg_color_mask, np.array(fg_mask_pil))
            mask2_np = np.array(fg_mask_pil)
        ...
        final_mask_tensor = final_mask_tensor.unsqueeze(0)
        mask2_tensor = torch.from_numpy(mask2_np).float() / 255.0
        mask2_tensor = mask2_tensor.unsqueeze(0)

        # 图像保存逻辑
        results = easySave(composed_tensor, 'composedPreview', image_output, prompt, extra_pnginfo)

        if image_output in ("Hide", "Hide/Save"):
            return {"ui": {}, "result": (composed_tensor, final_mask_tensor, mask2_tensor)}        
        return {"ui": {"images": results}, "result": (composed_tensor, final_mask_tensor, mask2_tensor)}
...
class XXcreate_Mask_match_shape: 
    ...
    def compose(self, bimg, bmask, b_color, b_extrant_to_block, f_extrant_to_block, 
                edge_detection, edge_thickness, edge_color,
                f_smoothness, f_opacity, image_output,
                fimg=None, fmask=None, f_scale=1.0, f_x_offset=0, f_y_offset=0, f_rot=0,
                prompt=None, extra_pnginfo=None):
        ...
        has_foreground = fimg is not None
        edge_image = None
        foreground_layer_pil = Image.new("RGBA", bimg_processed.size, (0, 0, 0, 0))  # 初始化前景图层
        foreground_mask_np = np.zeros_like(bg_color_mask)  # 初始化前景遮罩
        ...
        if has_foreground:
            ...
            if f_contours:
                rf_moments = [cv2.moments(cnt) for cnt in f_contours]
                rf_contour_centers = []
                for m in rf_moments:
                    if m["m00"] != 0:
                        cx = int(m["m10"] / m["m00"])
...
class XXXcreate_Mask_match_shape: 
    def __init__(self):
        self.bg_colors = {
            "black": (0, 0, 0),
            "white": (255, 255, 255),
    ...
    def get_min_rect(self, mask_np):
        ...
        if not contours:
            return (mask_np.shape[1], mask_np.shape[0])  # 无轮廓时返回掩码原始尺寸（W, H）
