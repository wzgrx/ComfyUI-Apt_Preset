...
class Data_Highway:
    ...
    RETURN_NAMES = ("context", *[f"output_{i}" for i in range(MAX_OUTPUTS)])
    DESCRIPTION = """
    ...
    def INPUT_TYPES(cls):
        return {
            ...
            "optional": {"context": ("RUN_CONTEXT2", {"default": None})}
        }
    ...
    def process(self, port_config, context=None, **kwargs):
        sections = port_config.split(";")
        input_section = sections[0].strip()
        output_section = sections[1].strip() if len(sections) > 1 else ""

        input_names = [p.strip() for p in input_section.split(">") if p.strip() and p.strip() != "context"]
        output_names = [p.strip().lstrip("<") for p in output_section.split("<") if p.strip() and p.strip() != "context"] or input_names.copy()

        packed = RevisionDict()
        packed[("kind")] = "Data_Highway"
        packed[("context")] = context

        if context is not None and isinstance(context, RevisionDict):
            for name in input_names:
                if ("data", name) in context._data:
                    if name not in kwargs or kwargs[name] is None:
                        kwargs[name] = context._data[("data", name)]
    ...
    def parse_ports(self, config):
        ...
        if sections[0].strip():
            self.input_ports = [p.strip() for p in sections[0].split('>') if p.strip() and p.strip() != "context"]

        if len(sections) > 1 and sections[1].strip():
            self.output_ports = [p.strip().lstrip('<') for p in sections[1].split('<') if p.strip() and p.strip() != "context"]
        else:
