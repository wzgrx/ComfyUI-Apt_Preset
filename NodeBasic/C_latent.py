...
class chx_latent_adjust:
    ...
    CATEGORY = "Apt_Preset/latent"
    def adjust_latent(self, context, brightness=1.0, contrast=1.0, saturation=1.0, exposure=0.0,
...
class chx_latent_blend:
    ...
    def latent_blend(self, context, operation, blend_ratio, op_strength, img_targe=None, 
                    img_blend=None, normalize=None, clamp_min=None, clamp_max=None, ):
        
        latent = context.get("latent", None)
        vae = context.get("vae", None)

        if img_blend is None:
            return (context, latent, None)
        if img_targe is None:
            img_targe = context.get("images", None)
        img_blend = get_image_resize(img_blend,img_targe)   

        latent_a = encode(vae, img_blend)[0]
        latent_b = encode(vae, img_targe)[0]
        ...
        if blended_latent.shape[1] != 4:
            blended_latent = blended_latent[:, :4]

        context = new_context(context, latent={"samples": blended_latent})
        return (context, {"samples": blended_latent})
