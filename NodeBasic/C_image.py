...
class Image_smooth_blur:
    ...
    RETURN_TYPES = ("IMAGE", "MASK")
    RETURN_NAMES = ("image", "smooth_mask")
    CATEGORY = "Apt_Preset/image"
...
class XXImage_Resize2:
    ...
    def resize(self, image, get_image_size,):
        if len(image.shape) == 3:
            H, W, C = image.shape
        else:  
            B, H, W, C = image.shape
        ...
        image = image.movedim(-1,1)
        image = common_upscale(image, width_max, height_max, "bicubic", "center")
        image = image.movedim(1,-1)
...
class Image_tensor_Converter:
    ...
    def convert_precision(self, image, convert_to_pil=True, multiple=8):
        batch_size = image.shape[0]
        converted_images = []
...
class Image_solo_stitch:
    ...
    CATEGORY = "Apt_Preset/image"
    RETURN_TYPES = ("IMAGE","IMAGE","IMAGE")  # 添加一个IMAGE返回类型
    RETURN_NAMES = ("image","recover_image","original_image")  # 添加original_image返回名称
...
class Image_solo_crop:
    ...
    def process_resize(self, image, mask, crop_mode, long_side, divisible_by, upscale_method="bilinear"):
        ...
        # 直接用F.interpolate缩放，不转PIL
        image_t = image.permute(0, 3, 1, 2)  # (B, C, H, W)
        ...
        crop_image = crop_image.permute(0, 2, 3, 1)  # (B, H, W, C)

        mask_t = mask.unsqueeze(1) if mask.ndim == 3 else mask  # (B, 1, H, W)
        crop_mask = F.interpolate(mask_t, size=(new_height, new_width), mode="nearest")
        crop_mask = crop_mask.squeeze(1)  # (B, H, W)
    ...
    def inpaint_crop(self, image, crop_mode, long_side, upscale_method="bilinear", 
        ...
        if crop_img_bj != "image" and crop_img_bj in colors:
            ...
            background = torch.zeros((crop_image.shape[0], h, w, 3), device=crop_image.device)
            background[:, :, :, 0] = r
            background[:, :, :, 1] = g
            background[:, :, :, 2] = b
        ...
        # 如果一边受限，则将多余的扩展加到另一边
        # 处理宽度方向的不对称扩展
        if ideal_x_new < 0:  # 左边受限
            additional_expand = abs(ideal_x_new)
            x_end = min(original_w, x_end + additional_expand)
        elif ideal_x_end > original_w:  # 右边受限
            additional_expand = ideal_x_end - original_w
            x_new = max(0, x_new - additional_expand)
        
        # 处理高度方向的不对称扩展
        if ideal_y_new < 0:  # 上边受限
            additional_expand = abs(ideal_y_new)
            y_end = min(original_h, y_end + additional_expand)
        elif ideal_y_end > original_h:  # 下边受限
            additional_expand = ideal_y_end - original_h
            y_new = max(0, y_new - additional_expand)

        # 确保尺寸符合 divisible_by 要求
        if divisible_by > 1:
        ...
        stitch = {
            ...
            "bj_image": bj_image,
            "original_image": image  # 添加原始图像到stitch字典
        }
...
class Image_pad_adjust_restore:
    ...
    def restore(self, pad_image, stitch):
        # 从stitch中提取信息
        original_image = stitch["original_image"]
        original_h, original_w = stitch["original_shape"]
        # 提取原始参数和实际调整值
        ...
        for i in range(batch_size):
            ...
            # 创建与原始尺寸相同的图像
            restored_img = orig_img.clone()  # 先复制原始图像
            ...
            # 计算裁剪和填充区域
            # 裁剪区域（负数参数）
            crop_left = max(-orig_left, 0)
            crop_top = max(-orig_top, 0)
...
class Image_pad_adjust:
    ...
    def INPUT_TYPES(s):
        return {
            "required": {
                "image": ("IMAGE",),
                "top": ("INT", {"default": 0, "step": 1, "min": -4096, "max": 4096}),
                "bottom": ("INT", {"default": 0, "step": 1, "min": -4096, "max": 4096}),
                "left": ("INT", {"default": 0, "step": 1, "min": -4096, "max": 4096}),
                "right": ("INT", {"default": 0, "step": 1, "min": -4096, "max": 4096}),
                "bg_color": (["white", "black", "red", "green", "blue", "gray"], {"default": "black"}),
                "smoothness": ("INT", {"default": 0, "step": 1, "min": 0, "max": 500}),
                "divisible_by": ("INT", {"default": 2, "min": 1, "max": 512, "step": 1}),
                "auto_pad": (["None", "auto", "target_WxH", "1024x1024", "2048x512", "512x2048"], {"default": "None"}),
                "target_W": ("INT", {"default": 512, "min": 1, "max": 4096, "step": 1}),
                "target_H": ("INT", {"default": 512, "min": 1, "max": 4096, "step": 1}),
                "pad_mask_remove": ("BOOLEAN", {"default": True,}),
            },
            "optional": {
                "mask": ("MASK",),
    ...
    def process(self, image, left, top, right, bottom, bg_color, smoothness, divisible_by, auto_pad, target_W, target_H, pad_mask_remove, mask=None):
        ...
        if auto_pad == "1024x1024":
            target_W, target_H = 1024, 1024
        elif auto_pad == "2048x512":
            target_W, target_H = 2048, 512
        elif auto_pad == "512x2048":
            target_W, target_H = 512, 2048

        if auto_pad == "auto":
            image, mask, left, top, right, bottom = self.auto_padding(image, mask, left, top, right, bottom)
        elif auto_pad in ["target_WxH", "1024x1024", "2048x512", "512x2048"]:
            image, mask, left, top, right, bottom = self.target_padding(image, mask, left, top, right, bottom, target_W, target_H)
        ...
        created_mask, padding_mask = self.create_mask(cropped_image, actual_left, actual_top, actual_right, actual_bottom, 
                                    smoothness, cropped_mask, divisible_by)
        
        # 根据pad_mask_remove参数决定输出哪个遮罩
        # 当pad_mask_remove为True时，移除填充部分，只保留原始图像区域的遮罩
        # 当pad_mask_remove为False时，返回完整的遮罩（原始遮罩+填充区域）
        if pad_mask_remove:
            # 当pad_mask_remove为True时，我们希望移除填充区域（padding_mask为1的部分）
            # 所以应该返回 created_mask * (1 - padding_mask)
            final_mask = created_mask * (1 - padding_mask)
        else:
            # 当pad_mask_remove为False时，返回完整的遮罩
            final_mask = created_mask
    ...
    def target_padding(self, image, mask, left, top, right, bottom, target_W, target_H):
        batch_size, height, width, _ = image.shape
        
        if target_W < width or target_H < height:
            raise ValueError(f" target size must be larger than current size.")
        
        pad_width = target_W - width
        pad_height = target_H - height
        new_right = right + pad_width
        new_bottom = bottom + pad_height
        return image, mask, left, top, new_right, new_bottom

    def auto_padding(self, image, mask, left, top, right, bottom):
        batch_size, height, width, _ = image.shape
