...
from .load_GGUF.nodes import  UnetLoaderGGUF
...
target_dir = os.path.join(custom_nodes_dir, 'ComfyUI-GGUF')
...
if not os.path.exists(target_dir):
    ...
    try:
        ...
        pass
"""
def check_UnetLoaderGGUF_installed():
...
available_clips = list(set(folder_paths.get_filename_list("text_encoders") + folder_paths.get_filename_list("clip")))
...
class sum_load_adv:
    ...
    def INPUT_TYPES(cls):
        ...
        return {
            "required": {
                ...
                "clip4": (["None"] + available_clips,),  # hidreams
    ...
    def process_settings(self, node_id, lora_strength, 
                        width, height, steps, cfg, sampler, scheduler, unet_Weight_Dtype, guidance, over_clip=None,  clip_type=None, device="default", 
                        vae=None, lora=None, unet_name=None, ckpt_name=None, clip1=None, lora_stack=None, over_model=None,
                        clip2=None, clip3=None, clip4=None, pos="default", neg="default", preset=[]):
...
#model-----------------------
        if over_model is not None:
            ...
            if unet_name.endswith(".gguf"):
                check_UnetLoaderGGUF_installed()
                loader = UnetLoaderGGUF()  # 创建实例
...
#clip-----------------------
        if over_clip is not None:
            ...
            raise ValueError("pls Input clip1, clip2, clip3, clip4 in sequence.")
        
#update model ,clip, pos, neg----------------------
...
class load_create_chx:
    ...
    RETURN_TYPES = ("RUN_CONTEXT","MODEL","CLIP",  ANY_TYPE )
    RETURN_NAMES = ("context","model","clip","data", )
    FUNCTION = "process_settings"
    CATEGORY = "Apt_Preset/chx_load"
    ...
    def process_settings(self, 
        ...
        return (context, model, clip, data)
