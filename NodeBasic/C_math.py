...
class math_UnaryOperation:
    ...
    FUNCTION = "op"
    CATEGORY = "Apt_Preset/math"

    def op(self, op: str, a: float) -> tuple[float, int]:
        try:
            out1 = FLOAT_UNARY_OPERATIONS[op](a)
            # 处理特殊值，避免转换异常
            if math.isnan(out1) or math.isinf(out1):
                out2 = 0
            else:
                out2 = int(out1)  # 直接截断小数部分
            return (out1, out2)
        except Exception as e:
            # 处理可能的异常
            print(f"Error performing operation {op} on {a}: {e}")
            return (0.0, 0)


class math_UnaryCondition:
    @classmethod
    def INPUT_TYPES(cls) -> Mapping[str, Any]:
        return {
            "required": {
...
class math_BinaryOperation:
    ...
    def op(self, op: str, a: float, b: float) -> tuple[float]:
        try:
            out1 = FLOAT_BINARY_OPERATIONS[op](a, b)
            # 处理特殊值，避免转换异常
            if math.isnan(out1) or math.isinf(out1):
                out2 = 0
            else:
            ...
            # 处理可能的异常
            print(f"Error performing operation {op} on {a} and {b}: {e}")
            return (0.0, 0)


class math_BinaryCondition:
    @classmethod
    def INPUT_TYPES(cls) -> Mapping[str, Any]:
        return {
            "required": {
...
class list_input_text:
    
    @classmethod
    def INPUT_TYPES(s):
        return {"required": {
            "text": ("STRING", {"multiline": True, "default": "girl@boy"}),
    ...
    def cycle(self, text, delimiter, repeats):
...
class list_input_Value:
    ...
    def INPUT_TYPES(s):
        return {"required": {
            "values": ("STRING", {"multiline": True, "default": "1@5"}),
            "delimiter": ('STRING', {"forceInput": False}, {"default": "@"}),
            "repeats": ("INT", {"default": 1, "min": 1, "max": 99999}),
            }
        }
    ...
    def cycle(self, values, delimiter, repeats):
        delimiter = delimiter.replace("\\n", "\n")
        lines = values.split(delimiter)
        float_list_out = []
        int_list_out = []
...
class batch_cycler_Prompt:
    ...
    def condition(self, clip, keyframe_list, current_frame, easing_type):      
        ...
        (current_prompt, next_prompt, weight) = batch_cycler_text().simple_schedule( keyframe_list, current_frame, easing_type)
...
class math_calculate:
    ...
    def INPUT_TYPES(s):
        return {
            "required": {
                "expression": ("STRING", {"default": "a", "multiline": False,"tooltip": """
    - 基本运算：加 (+)、减 (-)、乘 (*)、除 (/)、模 (%)
