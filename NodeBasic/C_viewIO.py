...
class IO_save_image:
    ...
    RETURN_TYPES = ("STRING", "STRING")
    RETURN_NAMES = ("out_path", "i_path")
    FUNCTION = "save_image"
    OUTPUT_NODE = True
    OUTPUT_IS_LIST = (True, False)  # 指定第一个返回值为列表类型，第二个为单个值
    CATEGORY = "Apt_Preset/View_IO"
    ...
    def save_image(self, image, file_format, filename_prefix="Apt", output_path="", index=0):
        ...
        # 返回全部路径列表 和 指定 index 的路径
        indexed_path = absolute_paths[index] if 0 <= index < len(absolute_paths) else ""

        return (absolute_paths, indexed_path)




class XIO_save_image: #单地址
    def __init__(self):
        self.type = "output"
    ...
    def save_image(self, image, file_format, filename_prefix="Apt", output_path="./output/Apt"):
        batch_size = image.shape[0]
        images_list = [image[i:i + 1, ...] for i in range(batch_size)]
        ...
        # 获取当前前缀下的最大序号
        counter = self.find_highest_numeric_value(output_path, filename_prefix) + 1
        ...
        for idx, img_tensor in enumerate(images_list):
            ...
            output_filename = f"{filename_prefix}_{counter + idx:05}"
            resolved_image_path = os.path.join(output_path, f"{output_filename}.{file_format}")
            ...
            results.append({
                "filename": f"{output_filename}.{file_format}",
                "subfolder": os.path.basename(output_path),
                "type": self.type
            })
