...
class XXXexcel_Prompter:    #非原始替换
    ...
    def load_config(cls) -> List[str]:
        ...
        try:
            ...
            xlsx_files = []
            # 从第一行开始读取第二列（B列），直到遇到空行为止
            row = 1
            while True:
                # 关键修改：将column=1改为column=2，读取第二列
    ...
    def execute(self, replace_prompt="", replace_object="", 
                pos_stack="", pre_pos="", back_pos="",
                custom_targets="", custom_replacements="", **kwargs):
        positive_prompt = pre_pos + " "
        negative_prompt = ""

        for file_key in self.excel_files:
            selected_value = kwargs.get(file_key)
            if selected_value in self.excel_data[file_key]:
                positive_prompt += self.excel_data[file_key][selected_value][0] + " "
                negative_prompt += self.excel_data[file_key][selected_value][1] + " "

        positive_prompt += back_pos + " "
...
class excel_Prompter:
    ...
    def INPUT_TYPES(cls):
        ...
        input_config = {
            ...
            "optional": {
                ...
                "custom_replacements": ("STRING", { "multiline": False, "default": "", })
                }
            }
        for file_key in cls.excel_files:
            input_config["required"][file_key] = (list(cls.excel_data[file_key].keys()),)
    ...
    def execute(self, replace_content="", replace_item="", 
        ...
        if custom_targets and custom_replacements:
            positive_prompt = self.multi_replace(positive_prompt, custom_targets, custom_replacements)
