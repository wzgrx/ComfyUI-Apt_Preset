...
class XXXexcel_VedioPrompt:   
    ...
    def INPUT_TYPES(cls):
        # 加载所有数据
        cls.style_data = cls.load_excel_sheet(cls.EXCEL_PATH, "风格化")
        cls.light_data = cls.load_excel_sheet(cls.EXCEL_PATH, "环境光")
        cls.scene_data = cls.load_excel_sheet(cls.EXCEL_PATH, "场景")
        cls.landscape_data = cls.load_excel_sheet(cls.EXCEL_PATH, "景观")
        ...
        if not cls.landscape_data:
            cls.landscape_data = {"默认景观": {'chinese': "城市景观", 'english': "urban landscape"}}
        ...
        return {
            "required": {
                ...
                "style": (list(cls.style_data.keys()), {"label": "风格化"}),
                "light": (list(cls.light_data.keys()), {"label": "环境光"}),
                "scene": (list(cls.scene_data.keys()), {"label": "场景"}),
                "landscape": (list(cls.landscape_data.keys()), {"label": "景观"}),
    ...
    def execute(self, style, light, scene, landscape, camera, atmosphere, template,
                language=True, object="", target="", custom=""):
        ...
        landscape_val = self.landscape_data.get(landscape, {}).get(lang, "城市景观" if language else "urban landscape")
        ...
        # 如果需要使用公式
        if use_formula:
            parts = [style_val, light_val, scene_val, landscape_val, camera_val, atmosphere_val]
...
class excel_roles:
    BASE_DIR = os.path.dirname(os.path.abspath(__file__))
    EXCEL_PATH = os.path.join(BASE_DIR, "special", "角色.xlsx")
