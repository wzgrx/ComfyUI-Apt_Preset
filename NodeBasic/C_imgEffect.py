        ...
        def process_image(input_image, caption_text):
            ...
            for word in words:
                ...
                if current_line_width + word_width <= width - 2 * text_x:
                    ...
                    current_line_width += word_width + font_obj.getbbox(" ")[2]  # Add space width
                else:
    ...
    def layout(self, long_size, template, out_text, input_text1, input_text2, input_text3, font_size, font,
        ...
        if font_size > 0:
            if out_img is not None and out_text != "":
                out_img = lay_image_add_text().addlabel(out_img, out_text, font_size, font)
            if input_img1 is not None and input_text1 != "":
                input_img1 = lay_image_add_text().addlabel(input_img1, input_text1, font_size, font)
            if input_img2 is not None and input_text2 != "":
                input_img2 = lay_image_add_text().addlabel(input_img2, input_text2, font_size, font)
            if input_img3 is not None and input_text3 != "":
                input_img3 = lay_image_add_text().addlabel(input_img3, input_text3, font_size, font)

        # 初始化 Nimage 以避免未定义错误
        Nimage = None

        # 根据模板和可用图像进行处理，支持降级逻辑
        if template == "H11":
            if input_img1 is not None:
                if out_img is not None:
                    result = lay_iamge_conbine().concatenate(out_img, input_img1, direction="right")
                    Nimage = result[0] if isinstance(result, tuple) else result
                else:
                    Nimage = input_img1
            else:
            ...
            # H12 降级逻辑: input_img2 -> input_img1 -> out_img
            if input_img1 is not None and input_img2 is not None:
                result = lay_iamge_conbine().concatenate(input_img1, input_img2, direction="up")
                Nimage = result[0] if isinstance(result, tuple) else result
                if out_img is not None:
                    result = lay_iamge_conbine().concatenate(out_img, Nimage, direction="right")
                    Nimage = result[0] if isinstance(result, tuple) else result
            elif input_img1 is not None:  # input_img2 缺失，降级到 H11
                if out_img is not None:
                    result = lay_iamge_conbine().concatenate(out_img, input_img1, direction="right")
                    Nimage = result[0] if isinstance(result, tuple) else result
                else:
            ...
            # H13 降级逻辑: input_img3 -> input_img2 -> input_img1 -> out_img
            if input_img1 is not None and input_img2 is not None and input_img3 is not None:
                result = lay_iamge_conbine().concatenate(input_img1, input_img2, direction="up")
                Nimage12 = result[0] if isinstance(result, tuple) else result
                result = lay_iamge_conbine().concatenate(input_img3, Nimage12, direction="up")
                Nimage = result[0] if isinstance(result, tuple) else result
                if out_img is not None:
                    result = lay_iamge_conbine().concatenate(out_img, Nimage, direction="right")
                    Nimage = result[0] if isinstance(result, tuple) else result
            elif input_img1 is not None and input_img2 is not None:  # input_img3 缺失，降级到 H12
                result = lay_iamge_conbine().concatenate(input_img1, input_img2, direction="up")
                Nimage = result[0] if isinstance(result, tuple) else result
                if out_img is not None:
                    result = lay_iamge_conbine().concatenate(out_img, Nimage, direction="right")
                    Nimage = result[0] if isinstance(result, tuple) else result
            elif input_img1 is not None:  # input_img2 也缺失，降级到 H11
                if out_img is not None:
                    result = lay_iamge_conbine().concatenate(out_img, input_img1, direction="right")
                    Nimage = result[0] if isinstance(result, tuple) else result
                else:
            ...
            if input_img1 is not None and input_img2 is not None:
                ...
                Nimage = result[0] if isinstance(result, tuple) else result
                if out_img is not None:
                    result = lay_iamge_conbine().concatenate(out_img, Nimage, direction="left")
                    Nimage = result[0] if isinstance(result, tuple) else result
            elif input_img1 is not None:  # input_img2 缺失，降级到单独使用 input_img1
                if out_img is not None:
                    result = lay_iamge_conbine().concatenate(out_img, input_img1, direction="left")
                    Nimage = result[0] if isinstance(result, tuple) else result
                else:
                    Nimage = input_img1
            else:
            ...
            if input_img1 is not None and input_img2 is not None and input_img3 is not None:
                ...
                if out_img is not None:
                    result = lay_iamge_conbine().concatenate(out_img, Nimage, direction="left")
                    Nimage = result[0] if isinstance(result, tuple) else result
            elif input_img1 is not None and input_img2 is not None:  # input_img3 缺失
                result = lay_iamge_conbine().concatenate(input_img1, input_img2, direction="up")
                Nimage = result[0] if isinstance(result, tuple) else result
                if out_img is not None:
                    result = lay_iamge_conbine().concatenate(out_img, Nimage, direction="left")
                    Nimage = result[0] if isinstance(result, tuple) else result
            elif input_img1 is not None:  # input_img2 也缺失
                if out_img is not None:
                    result = lay_iamge_conbine().concatenate(out_img, input_img1, direction="left")
                    Nimage = result[0] if isinstance(result, tuple) else result
                else:
            ...
            if input_img1 is not None:
                if out_img is not None:
                    result = lay_iamge_conbine().concatenate(out_img, input_img1, direction="up")
                    Nimage = result[0] if isinstance(result, tuple) else result
                else:
                    Nimage = input_img1
            else:
            ...
            if input_img1 is not None and input_img2 is not None:
                result = lay_iamge_conbine().concatenate(input_img1, input_img2, direction="right")
                Nimage = result[0] if isinstance(result, tuple) else result
                if out_img is not None:
                    result = lay_iamge_conbine().concatenate(out_img, Nimage, direction="up")
                    Nimage = result[0] if isinstance(result, tuple) else result
            elif input_img1 is not None:  # input_img2 缺失，降级到 V11
                if out_img is not None:
                    result = lay_iamge_conbine().concatenate(out_img, input_img1, direction="up")
                    Nimage = result[0] if isinstance(result, tuple) else result
                else:
            ...
            if input_img1 is not None and input_img2 is not None and input_img3 is not None:
                result = lay_iamge_conbine().concatenate(input_img1, input_img2, direction="right")
                Nimage12 = result[0] if isinstance(result, tuple) else result
                result = lay_iamge_conbine().concatenate(input_img3, Nimage12, direction="right")
                Nimage = result[0] if isinstance(result, tuple) else result
                if out_img is not None:
                    result = lay_iamge_conbine().concatenate(out_img, Nimage, direction="up")
                    Nimage = result[0] if isinstance(result, tuple) else result
            elif input_img1 is not None and input_img2 is not None:  # input_img3 缺失，降级到 V12
                result = lay_iamge_conbine().concatenate(input_img1, input_img2, direction="right")
                Nimage = result[0] if isinstance(result, tuple) else result
                if out_img is not None:
                    result = lay_iamge_conbine().concatenate(out_img, Nimage, direction="up")
                    Nimage = result[0] if isinstance(result, tuple) else result
            elif input_img1 is not None:  # input_img2 也缺失，降级到 V11
                if out_img is not None:
                    result = lay_iamge_conbine().concatenate(out_img, input_img1, direction="up")
                    Nimage = result[0] if isinstance(result, tuple) else result
                else:
            ...
            if input_img1 is not None and input_img2 is not None:
                ...
                Nimage = result[0] if isinstance(result, tuple) else result
                if out_img is not None:
                    result = lay_iamge_conbine().concatenate(out_img, Nimage, direction="down")
                    Nimage = result[0] if isinstance(result, tuple) else result
            elif input_img1 is not None:  # input_img2 缺失
                if out_img is not None:
                    result = lay_iamge_conbine().concatenate(out_img, input_img1, direction="down")
                    Nimage = result[0] if isinstance(result, tuple) else result
                else:
                    Nimage = input_img1
            else:
        ...
        # 确保 Nimage 是 tensor 格式，然后转换为 PIL Image
        if isinstance(Nimage, torch.Tensor):
            ...
            # 如果是其他类型，尝试转换或使用默认图像
            try:
                Nimage_pil = tensor2pil(torch.zeros(1, 512, 512, 3))
            except:
