...
class DynamicTileSplit:
    ...
    def tile_image(self, image, width_factor, height_factor, overlap_rate,):
...
class pre_QwenEdit2:
    @classmethod
    def INPUT_TYPES(cls):
        default_template = qwen_images_en
        return {
            "required": {
                "context": ("RUN_CONTEXT",),
            },
    ...
    def qwen_encode(self, clip, prompt, vae=None, image=None, system_template=None):
    ...
    def process(self, context=None, image=None, mask=None, pos="", smoothness=0, model_shift=3.0, image_latent=None, system_template=None):  
        # 添加了 system_template=None 参数
        ...
        if image is None and image_latent is not None:
            image = context.get("images", None)
            if  image is None:
                return (context, None, None)
        ...
        positive =self.qwen_encode(clip, pos, vae, image, system_template)  # 传递 system_template 参数
        negative =self.qwen_encode(clip, "", vae, image, system_template)   # 传递 system_template 参数
...
class QwenImageTokenizer(sd1_clip.SD1Tokenizer):    # 更新调用
    def __init__(self, embedding_directory=None, tokenizer_data={}):
        ...
        self.llama_template_images_en = qwen_images_en
        self.llama_template_images_zh = qwen_images_zh
