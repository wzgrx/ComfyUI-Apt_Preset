...
class Data_preset_save:
    ...
    def saveparam(self, param, tomlname, savetype):
        ...
        tomltext += f"device_id = \"{param[0]['device_id']}\"\n"
        tomltext += f"data_type = \"{param[0]['data_type']}\"\n"
        tomltext += f"i2f_mode = \"{param[0]['i2f_mode']}\"\n"
...
try:
    ...
    print("模块中未找到 NunchakuFluxDiTLoader 类")
    NunchakuFluxDiTLoader = None
...
class load_Nanchaku:
    ...
    def INPUT_TYPES(cls):
        ...
        return {
            "required": {
                ...
                "attention": (attention_options, {"default": attention_options[0],}),
                "cpu_offload": (["auto", "enable", "disable"], {"default": "auto",}),
                "device_id": ("INT", {"default": 0, "min": 0, "max": ngpus - 1, "step": 1, "display": "number", "lazy": True,}),
                "data_type": (dtype_options, {"default": dtype_options[0],}),
                "i2f_mode": (["enabled", "always"], {"default": "enabled",}),
                "clip1": (available_clips,{"default": "clip_l.safetensors"} ),  
                "clip2": (available_clips, {"default": "t5xxl_fp8_e4m3fn.safetensors"} ), 
                "guidance": ("FLOAT", {"default": 3.5, "min": 0.0, "max": 100.0, "step": 0.1}),
                "vae": (available_vaes,{"default": "ae.safetensors"} ),
    ...
    def process_settings(self, node_id, width, height, steps, cfg, sampler, scheduler, guidance, device="default", i2f_mode="enabled",
                        vae=None, clip1=None, unet_name=None, attention=None, cache_threshold=None, cpu_offload=None, device_id=None, data_type=None, lora_stack=None,
        ...
        #model = NunchakuFluxDiTLoader().load_model( unet_name,attention,cache_threshold,cpu_offload,device_id,data_type,**kwargs, )[0]

        if NunchakuFluxDiTLoader is not None:
            if callable(NunchakuFluxDiTLoader):
                model_loader = NunchakuFluxDiTLoader()
                model = model_loader.load_model(unet_name, attention, cache_threshold, cpu_offload, device_id, data_type, **kwargs)[0]
            else:
                model = NunchakuFluxDiTLoader.load_model(unet_name, attention, cache_threshold, cpu_offload, device_id, data_type, **kwargs)[0]
        else:
