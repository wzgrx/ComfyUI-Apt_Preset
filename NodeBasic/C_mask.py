    ...
    def INPUT_TYPES(s):
        return {
            "required": {
                ...
                "out_heigh": ("INT", {"default": 1024, "max": 88888, "min": 258, "step": 8, "tooltip": "height = 0 means not set"}),
            },
            "optional": {
                "img1": ("IMAGE",),
                "mask1": ("MASK",),
...
class Mask_split_mulMask:   # 无需序号输入
    def __init__(self):
        pass
    ...
    def separate(self, mask, ignore_threshold=100):
        ...
        # 处理排序后的轮廓，最多处理5个
        segmented_masks = []
        for contour in sorted_contours[:5]:  # 只处理前5个轮廓
            area = cv2.contourArea(contour)
            if area < ignore_threshold:
                continue
            segmented_mask = np.zeros_like(binary_mask)
