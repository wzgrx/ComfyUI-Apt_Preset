...
class Stack_pre_Mark:
    ...
    def visualize(self, bg_mode, mask_mode, output_mask, 
                  ignore_threshold, opacity, outline_thickness, 
                  smoothness, expand_width, expand_height, 
                  tapered_corners, sum_mask_min, sum_mask_max):
        # 打包所有参数到mark_stack元组中
        mark_stack = (
            bg_mode, mask_mode, output_mask,
            ignore_threshold, opacity, outline_thickness,
            smoothness, expand_width, expand_height,
            tapered_corners, sum_mask_min, sum_mask_max
        )
