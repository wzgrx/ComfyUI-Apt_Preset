...
class Image_solo_stitch:
    ...
    CATEGORY = "Apt_Preset/image"
    RETURN_TYPES = ("IMAGE","IMAGE",)  
    RETURN_NAMES = ("image","recover_image",)
    ...
    def Image_Channel_Apply(self, images: torch.Tensor, channel, invert_channel=False, 
        ...
        for image in images:
            ...
            if input_provided:
                ...
                if input_mask.shape[2:] != base_mask.shape:
                    input_mask = torch.nn.functional.interpolate(
                        input_mask,
                        size=base_mask.shape,
                        mode='bilinear',
                        align_corners=False
                    )
...
class XXXImage_target_adjust:
    def __init__(self):
        pass
    
    @classmethod
    def INPUT_TYPES(cls):  # 修正为大写
        return {
            "required": {
                "image": ("IMAGE",),
                ...
                "adjustment_method": (["stretch", "center_crop", "center_pad"], {
                    "default": "stretch"
                }),
            }
        }
    
    RETURN_TYPES = ("IMAGE",)  # 修正为大写
    FUNCTION = "resize_image"  # 修正为大写
    CATEGORY = "Apt_Preset/image"
    ...
    def resize_image(self, image, target_width, target_height, multiple, upscale_method, adjustment_method):
        ...
        def resize_fn(img):
            ...
            elif adjustment_method == "center_crop":
            ...
            elif adjustment_method == "center_pad":
                if original_aspect > target_aspect:
                    scale = output_width / original_width
                    scaled_width = output_width
                    scaled_height = int(original_height * scale)
                    ...
                    pad_bottom = output_height - scaled_height - pad_top
                    resized_pil = ImageOps.expand(resized, (0, pad_top, 0, pad_bottom), fill=0)
                else:
                    scale = output_height / original_height
                    scaled_width = int(original_width * scale)
                    scaled_height = output_height
                    ...
                    pad_right = output_width - scaled_width - pad_left
                    resized_pil = ImageOps.expand(resized, (pad_left, 0, pad_right, 0), fill=0)
...
class Image_pad_adjust:
    ...
    RETURN_TYPES = ("IMAGE", "MASK")
    RETURN_NAMES = ("image", "mask")
    FUNCTION = "process"  # 重命名函数更符合功能
    CATEGORY = "Apt_Preset/image"

    def process(self, image, left, top, right, bottom, bg_color, smoothness, mask=None):
        # 处理图像：先裁切（如果参数为负），再填充（如果参数为正）
        cropped_image, cropped_mask = self.crop_image(image, mask, left, top, right, bottom)
        padded_image = self.add_padding(cropped_image, max(left, 0), max(top, 0), max(right, 0), max(bottom, 0), bg_color)
        created_mask = self.create_mask(cropped_image, max(left, 0), max(top, 0), max(right, 0), max(bottom, 0), smoothness, cropped_mask)
        
        return (padded_image, created_mask)
