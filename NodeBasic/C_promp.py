...
class XXexcel_insert_image:
    ...
    def write_image(self, excel_path, sheet_name, row_start, row_end, col_start, col_end, image_path):
        ...
        # 文件存在性及权限检查
        if not os.path.exists(excel_path):
            raise Exception(f"Error: Excel file does not exist at path: {excel_path}")
        if not os.access(excel_path, os.W_OK):
            raise Exception(f"Error: No write permission for Excel file at path: {excel_path}")
        if not os.path.exists(image_path):
            raise Exception(f"Error: Image file does not exist at path: {image_path}")
        if not os.access(image_path, os.R_OK):
            raise Exception(f"Error: No read permission for image file at path: {image_path}")
        ...
        try:
            ...
            # 调整单元格尺寸以适应图片，固定宽度为 256
            column_letter = get_column_letter(target_col)
            sheet.column_dimensions[column_letter].width = 256 / 7  # 固定宽度为 256
            ...
            raise Exception(f"Permission Error: {str(pe)}")
        except Exception as e:
            raise Exception(f"Error: {str(e)}")
