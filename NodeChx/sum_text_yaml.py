...
class text_sum:
    ...
    def INPUT_TYPES(cls):
        ...
        return {
            "required": {
                ...
                "replace_all": ("STRING", {"multiline": False,"default": ""}),
            },
    ...
    DESCRIPTION = """
- 替换内容，移除内容：多个词汇同时进行，用逗号隔开。例如："a, b, c"
- 全部替换：一段文章仅替换一个目标词
- """
    ...
    def run(self, style="default", negative="", replace_target="", replace="",remove="", add_pos="", add_neg="",replace_all="",  select_yaml: str = "", select: str = "", prompt: str = "", unique_id: str = "") -> Tuple[str]:
        ...
        if replace_target is not None and replace_target!= "": 
            if replace_all is not None and replace_all!= "":  prompt = prompt.replace(replace_target, replace_all)
            else: prompt = replace_text(prompt, replace_target, replace)
